...
function submodular_width(
    ...
    cis::Vector{Tuple{Set{T},Set{T}}} = Tuple{Set{T},Set{T}}[],
    verbose::Bool = false,
) where T
    n = length(H.vars)
    N = 2 ^ n
    ...
    for β in selectors
        ...
        for (i, edge) in enumerate(H.edges)
            ...
            E = zip(H, edge)
            c = @constraint(model, h[E] ≤ H.weights[i])
            cn = "       $(f(E)) ≤ $(H.weights[i])"
            d[cn] = c
        end
        ...
        for dc ∈ dcs
            ...
            X = zip(H, dc.X)
            Y = zip(H, dc.Y)
            c = @constraint(model, h[Y] - h[X] ≤ dc.n)
            cn = "     $(f(Y)) - $(f(X)) ≤ $(dc.n)"
            d[cn] = c
        end
        ...
        e = Dict(name => dual(c) for (name, c) ∈ d)
...
function _td_from_var_order(
    ...
    bags = Set{Set{T}}()
    for v in var_order
        v_edges = Set(e for e in edges if v in e)
        bag = reduce(union!, v_edges; init = Set{T}())
        push!(bags, bag)
        setdiff!(edges, v_edges)
        new_edge = setdiff(bag, Set((v,)))
        push!(edges, new_edge)
    end
    non_redundant_bags = Set{Set{T}}()
    for bag1 in bags
        is_redundant = false
        for bag2 in bags
            if bag2 != bag1 && issubset(bag1, bag2)
                is_redundant = true
                break
            end
        end
        !is_redundant && push!(non_redundant_bags, bag1)
    end
    return non_redundant_bags
end
...
function get_free_connex_tds(
    ...
    vars = collect(union(edges...,))
    vars2 = collect(F)
    vars1 = [v for v in vars if v ∉ F]
    # for each permutation `var_order` of the variables
    for var_order1 in permutations(vars1), var_order2 in permutations(vars2)
        var_order = [var_order1; var_order2]
        # construct a tree decomposition by eliminating variables using `var_order`
        td = _td_from_var_order(edges, var_order)
        # duplicate tree decompositions are automatically removed because we are storing
        # TDs in a set
...
# Q(BDF) :- R(ABC),R(CDE),R(EFA)

# function dan_formula(out)
#     return max(
#         4/3 + 1/3 * out,
#         1 + 2/3 * out,
#         # 5/4 + 1/2 * out,
...
#     tds = get_free_connex_tds(H.edges, Set(["B", "D", "F"]))
...
#     cis = Vector{Tuple{Set{String},Set{String}}}()

#     # _f(j::Int) = H.vars[(j - 1) % 6 + 1]

#     # for i = 1:6
#     #     push!(cis, (
#     #         Set{String}((_f(i), _f(i + 1), _f(i + 2), _f(i + 3), _f(i + 4))),
#     #         Set{String}((_f(i + 3), _f(i + 4), _f(i + 5), _f(i), _f(i + 1)))
#     #     ))
#     # end

#     push!(cis, (Set(["A", "B", "C"]), Set(["C", "D", "E", "F", "A"])))
#     push!(cis, (Set(["C", "D", "E"]), Set(["E", "F", "A", "B", "C"])))
#     push!(cis, (Set(["E", "F", "A"]), Set(["A", "B", "C", "D", "E"])))

#     println("$out\t$(dan_formula(out))\t$(submodular_width(H; cis = cis))")
# end
...
println(polymatroid_bound(H; fds))
